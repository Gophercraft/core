...
func (cl *Client) HandleCharList(b []byte) {
	...
	log.Warn(spew.Sdump(data))
	if len(b) > 100 {
		log.Warn("Weird packet: ", b)
	}
...
func (cl *Client) HandleCompressedUpdate(d []byte) {
	...
	log.Println("Compressed: ", len(body))
	log.Println("Decompressed: ", len(decompressedBody))
	log.Println("Sized: ", length)
	log.Println("packet should contain", in.Len()-4)
	// log.Fatal(hex.EncodeToString(d))
...
func (cl *Client) HandleUpdateData(d []byte) {
	...
	for _, v := range s.Blocks {
		...
		case packet.UPDATETYPE_CREATE_OBJECT:
			log.Println("Creating", v.GUID)

		case packet.UPDATETYPE_CREATE_OBJECT2:
			log.Println("Spawn", v.GUID)

			if v.GUID == cl.PlayerGUID {
				log.Fatal(spew.Sdump(v))
			}
		}
	}
}
