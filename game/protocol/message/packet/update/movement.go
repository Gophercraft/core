...
var (
	UpdateFlagDescriptors = map[version.BuildRange]UpdateFlagDescriptor{
		version.Range(0, 5875): {
	...
	MoveFlagDescriptors = map[version.BuildRange]MoveFlagDescriptor{
		version.Range(0, 3368): {
			MoveFlagForward:         0x1,
		...
		version.Range(version.V1_12_1, 6005): {
		...
		version.Range(version.V2_0_1, version.V2_4_3): {
			MoveFlagForward:         0x00000001,
			MoveFlagBackward:        0x00000002,
			MoveFlagStrafeLeft:      0x00000004,
			MoveFlagStrafeRight:     0x00000008,
		...
		version.Range(version.V3_0_2, version.V3_3_5a): {
			MoveFlagForward:         0x00000001,
			MoveFlagBackward:        0x00000002,
			MoveFlagStrafeLeft:      0x00000004,
			MoveFlagStrafeRight:     0x00000008,
	...
	SpeedLists = map[version.BuildRange][]SpeedType{
		...
		version.Range(version.V3_0_2, version.Max): {
			Walk,
			Run,
			RunBackward,
			Swim,
...
func GetSpeedList(v version.Build) SpeedList {
	var sp []SpeedType
	if err := version.QueryDescriptors(v, SpeedLists, &sp); err != nil {
		panic(err)
	}
...
func decodeUpdateFlags(version version.Build, in io.Reader) (UpdateFlags, error) {
	var descriptor UpdateFlagDescriptor
	if err := version.QueryDescriptors(version, UpdateFlagDescriptors, &descriptor); err != nil {
		return 0, err
	}

	var updateFlags uint32
	if version.AddedIn(version.V3_3_5a) {
...
func encodeUpdateFlags(version version.Build, outb io.Writer, uf UpdateFlags) error {
	var descriptor UpdateFlagDescriptor
	if err := version.QueryDescriptors(version, UpdateFlagDescriptors, &descriptor); err != nil {
		return err
	}
...
func decodeMoveFlags(version version.Build, in io.Reader) (MoveFlags, error) {
	var descriptor MoveFlagDescriptor

	if err := version.QueryDescriptors(version, MoveFlagDescriptors, &descriptor); err != nil {
		return 0, err
	}
	...
	if version.AddedIn(version.V2_0_1) {
		if version.RemovedIn(version.V3_0_2) {
			flagsSize += 1
		} else {
			flagsSize += 2
		}
...
func encodeMoveFlags(version version.Build, outb io.Writer, mf MoveFlags) error {
	var descriptor MoveFlagDescriptor
	...
	// Byte flags
	if version.AddedIn(version.V2_0_1) && version.RemovedIn(version.V3_0_2) {
		flagsSize = 5
	}

	// Uint16 flags
	if version.AddedIn(version.V3_0_2) {
		flagsSize = 6
	}
...
func EncodeMovementInfo(version version.Build, out io.Writer, mi *MovementInfo) error {
	// Alpha uses a simpler format
	if version.Range(0, version.Alpha).Contains(version) {
...
func DecodeMovementInfo(version version.Build, in io.Reader) (*MovementInfo, error) {
	info := new(MovementInfo)
	var err error
	// Alpha uses a very strange format.
	if version.RemovedIn(version.V1_12_1) {
	...
	// if version.AddedIn(version.V2_4_3) {
...
func (mb *MovementBlock) WriteData(e *Encoder, mask VisibilityFlags, create bool) error {
	...
	if mb.UpdateFlags&UpdateFlagLiving != 0 {
		...
		var sl []SpeedType
		if err := version.QueryDescriptors(e.Build, SpeedLists, &sl); err != nil {
			return err
		}
		...
		if e.Build.AddedIn(version.V3_3_5a) {
	...
	if e.Build.RemovedIn(version.V2_4_3) {
...
func (mp *MovementPacket) Encode(build version.Build, out *message.Packet) error {
	out.Type = mp.Type
	if build.AddedIn(version.V3_0_2) || mp.Server {
		mp.GUID.EncodePacked(build, out)
	}
	return EncodeMovementInfo(build, out, mp.Info)
}

func (mp *MovementPacket) Decode(build version.Build, in *message.Packet) error {
	var err error
	mp.Type = in.Type
	if build.AddedIn(version.V3_0_2) || mp.Server {
...
func (f *FlyState) Encode(build version.Build, out *message.Packet) error {
	if f.On {
		out.Type = message.SMSG_MOVE_SET_CAN_FLY
	} else {
		out.Type = message.SMSG_MOVE_UNSET_CAN_FLY
	}
...
func (f *FlyState) Decode(build version.Build, in *message.Packet) error {
...
// func (sc *SpeedChange) Encode(build version.Build, out *message.Packet) error {
...
// func (sc *SpeedChange) Decode(build version.Build, in *message.Packet) error {
// 	t := in.Type
