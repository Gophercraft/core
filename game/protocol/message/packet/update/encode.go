...
func NewEncoder(version version.Build, writer io.Writer, numBlocks int) (*Encoder, error) {
	e := &Encoder{
		Writer: writer,
		Build:  version,
	}
	err := version.QueryDescriptors(version, Descriptors, &e.Descriptor)
	if err != nil {
		return nil, fmt.Errorf("update: problem looking for descriptor, %s: %s", version, err)
	}
	if err := version.QueryDescriptors(version, BlockTypeDescriptors, &e.blockTypes); err != nil {
...
func (enc *Encoder) alpha() bool {
	return version.Range(0, version.Alpha).Contains(enc.Build)
}
