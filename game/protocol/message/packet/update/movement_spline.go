...
var (
	SplineFlagDescriptors = map[version.BuildRange]SplineFlagDescriptor{
		{0, 5875}: {
		...
		{version.V2_0_1, version.V3_3_5a}: {
			// x00-xFF(first byte) used as animation Ids storage in pair with Animation flag
...
func decodeSplineFlags(version version.Build, in io.Reader) (SplineFlags, error) {
	var descriptor SplineFlagDescriptor
	err := version.QueryDescriptors(version, SplineFlagDescriptors, &descriptor)
	if err != nil {
...
func encodeSplineFlags(version version.Build, out io.Writer, sf SplineFlags) error {
	var descriptor SplineFlagDescriptor
	err := version.QueryDescriptors(version, SplineFlagDescriptors, &descriptor)
	if err != nil {
...
func DecodeMoveSpline(version version.Build, in io.Reader) (*MoveSpline, error) {
	ms := new(MoveSpline)
	var err error
...
func EncodeMoveSpline(version version.Build, out io.Writer, ms *MoveSpline) error {
