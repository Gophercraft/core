...
func (decoder *Decoder) DecodeValuesBlockData(valuesBlock *ValuesBlock) error {
	...
	if !valuesBlock.StorageDescriptor.IsValid() {
		...
		if objectDescriptor == nil {
			log.Dump("objectInstance.Interface()", objectInstance.Interface())
			return fmt.Errorf("update: %s: no object descriptor for the received typemask: %s", decoder.Build, typeMask)
		}
...
func (valdec *ValuesDecoder) Decode(field reflect.Value, name string) error {
	...
	// Padded chunks should always be empty, as they do nothing other than to align the number of bits in memory.
	if field.Type() == chunkPadType {
		if valdec.CurrentBitmask.Enabled(valdec.ChunkPos) {
