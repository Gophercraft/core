...
func (smi *ServerModuleInitialize) Decode(build version.Build, in *Reader) error {
	var size uint16
	var checksum uint32
...
func (smi *ServerModuleInitialize) Encode(build version.Build, out *Writer) error {
	data := etc.NewBuffer()
	data.WriteUint16(smi.SizeFlags)
	switch smi.SizeFlags {
	case 1:
		data.WriteUint8(smi.Type)
		data.WriteUint8(smi.StringLibrary)
	case 4, 257:
		data.WriteUint8(smi.StringLibrary)
	}
