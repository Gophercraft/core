...
var InvResultDescriptors = map[version.BuildRange]InvResultDescriptor{
	{0, version.V8_3_0}: {
		InvOk:                            0,
...
func (ice *InvChangeError) Encode(build version.Build, out *message.Packet) error {
	out.Type = message.SMSG_INVENTORY_CHANGE_FAILURE

	var desc InvResultDescriptor
	if err := version.QueryDescriptors(build, InvResultDescriptors, &desc); err != nil {
		return err
	}
	...
	out.WriteUint8(ice.BagSubclass) //bag type subclass
...
func (ice *InvChangeError) Decode(build version.Build, in *message.Packet) error {
	var desc InvResultDescriptor
	if err := version.QueryDescriptors(build, InvResultDescriptors, &desc); err != nil {
		return err
	}
