...
func (q *Query) Decode(build version.Build, in *message.Packet) (err error) {
	q.Entry = in.ReadUint32()
	q.GUID, err = guid.DecodeUnpacked(build, in)
	return
}

func (q *Query) Encode(build version.Build, out *message.Packet) (err error) {
	out.Type = message.CMSG_CREATURE_QUERY
...
func (q *QueryResponse) Encode(build version.Build, out *message.Packet) error {
	out.Type = message.SMSG_CREATURE_QUERY_RESPONSE
