...
var MsgTypeDescriptors = map[version.BuildRange]MsgTypeDescriptor{
...
func ConvertMsgType(build version.Build, value MsgType) (uint32, error) {
	var desc MsgTypeDescriptor
	if err := version.QueryDescriptors(build, MsgTypeDescriptors, &desc); err != nil {
		return 0, err
	}
...
func ResolveMsgType(build version.Build, code uint32) (MsgType, error) {
	var desc MsgTypeDescriptor
	if err := version.QueryDescriptors(build, MsgTypeDescriptors, &desc); err != nil {
		return 0, err
	}
...
func (e *Error) Encode(build version.Build, out *message.Packet) error {
	out.Type = e.Type
	out.WriteCString(e.Name)
	return nil
}

func (e *Error) Decode(build version.Build, in *message.Packet) error {
	e.Type = in.Type
