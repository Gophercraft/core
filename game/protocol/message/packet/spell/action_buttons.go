...
func NumActionButtons(build version.Build) int {
	ab := 120
...
func (ab ActionButton) Uint32(build version.Build) uint32 {
...
func (abs *ActionButtons) Encode(build version.Build, out *message.Packet) error {
	out.Type = message.SMSG_ACTION_BUTTONS
...
func (abs *ActionButtons) Decode(build version.Build, in *message.Packet) error {
	nab := in.Len() / 4
...
func (sab *SetActionButton) Encode(build version.Build, out *message.Packet) error {
	out.Type = message.CMSG_SET_ACTION_BUTTON
	out.WriteUint8(uint8(sab.Slot))
	// out.WriteUint32(sab.Action&spellMask | (sab.Type << 24))
	out.WriteUint32(sab.ActionButton.Uint32(build))
	return nil
}

func (sab *SetActionButton) Decode(build version.Build, in *message.Packet) (err error) {
	sab.Slot = uint32(in.ReadUint8())
