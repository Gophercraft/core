...
func (cr *CastResult) Encode(build version.Build, out *message.Packet) error {
	out.Type = message.SMSG_CAST_RESULT
	if build.AddedIn(version.V3_0_2) {
		out.WriteUint8(cr.CastID)
	}
	out.WriteUint32(cr.SpellID)
	out.WriteUint8(uint8(cr.Status))
	if build.AddedIn(version.V3_0_2) {
		out.WriteUint8(cr.CastCount)
	}
	return nil
}
