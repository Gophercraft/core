...
func EncodeResult(build vsn.Build, out io.Writer, pr Result) error {
	...
	if err := vsn.QueryDescriptors(build, ResultDescriptors, &desc); err != nil {
		return err
	}
...
func DecodeResult(build vsn.Build, in io.Reader) (Result, error) {
	var desc ResultDescriptor
	if err := vsn.QueryDescriptors(build, ResultDescriptors, &desc); err != nil {
		return 0, err
	}
...
func (p *RequestMemberStats) Encode(build vsn.Build, out *packet.WorldPacket) error {
	out.Type = packet.CMSG_REQUEST_PARTY_MEMBER_STATS
...
func writeHealth(build vsn.Build, value uint32, out *packet.WorldPacket) {
...
func readHealth(build vsn.Build, in *packet.WorldPacket) uint32 {
	if build.AddedIn(vsn.V2_0_1) {
...
func writeAuras(build vsn.Build, auras []MemberAuraState, out *packet.WorldPacket) {
	var mask uint64
	...
	for i, aura := range auras {
		if mask&(1<<uint64(i)) != 0 {
			switch {
			case build < vsn.V2_0_1:
				out.WriteUint16(uint16(aura.SpellID))
			case vsn.Range(vsn.V2_0_1, vsn.V3_3_5a).Contains(build):
				out.WriteInt32(aura.SpellID)
				out.WriteByte(uint8(aura.Flags))
			default:
...
func readAuras(build vsn.Build, in *packet.WorldPacket) []MemberAuraState {
	var mask uint64
	var maskLen int
	if build.AddedIn(vsn.V2_0_1) {
		mask = in.ReadUint64()
	...
	for i := 0; i < maskLen; i++ {
		if mask&(1<<uint64(i)) != 0 {
			...
			case vsn.Range(vsn.V2_0_1, vsn.V3_3_5a).Contains(build):
				state.SpellID = in.ReadInt32()
				state.Flags = uint16(in.ReadByte())
			default:
...
func (p *MemberStats) Encode(build vsn.Build, out *packet.WorldPacket) error {
	out.Type = packet.SMSG_PARTY_MEMBER_STATS_FULL
	if build.AddedIn(vsn.V2_0_1) {
		out.WriteByte(0)
	}
	...
	if p.Change(GroupUpdatePetPowerType) {
		out.WriteByte(uint8(p.PetPowerType))
	}
...
func (p *MemberStats) Decode(build vsn.Build, in *packet.WorldPacket) error {
	...
	if p.Change(GroupUpdateStatus) {
		p.Status = MemberStatus(in.ReadByte())
	}
	if p.Change(GroupUpdateCurrentHealth) {
	...
	if p.Change(GroupUpdatePetPowerType) {
		p.PetPowerType = uint16(in.ReadByte())
	}
	if p.Change(GroupUpdatePetCurrentPower) {
...
func (pcr *CommandResult) Encode(build vsn.Build, out *packet.WorldPacket) error {
	out.Type = packet.SMSG_PARTY_COMMAND_RESULT
