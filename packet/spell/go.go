...
func (g *Go) Encode(build vsn.Build, out *packet.WorldPacket) (err error) {
	...
	if build.AddedIn(9056) {
		...
		if g.Flags&RuneInfo != 0 {
			out.WriteByte(g.SpellRuneState)
			out.WriteByte(g.PlayerRuneState)
			...
			for b := uint8(0); b < uint8(len(g.RuneCooldownPasses)); b++ {
				...
				out.WriteByte(g.RuneCooldownPasses[int(b)])
			}
		}
		...
		if build.AddedIn(9056) {
			...
			if g.TargetFlags&DestinationLocation != 0 {
				out.WriteByte(g.DestinationLocationCount)
			}
...
func (g *Go) Decode(build vsn.Build, in *packet.WorldPacket) (err error) {
	...
	if build.AddedIn(9056) {
		...
		if g.Flags&RuneInfo != 0 {
			g.SpellRuneState = in.ReadByte()
			g.PlayerRuneState = in.ReadByte()
			...
			for b := uint8(0); b < uint8(len(g.RuneCooldownPasses)); b++ {
				...
				g.RuneCooldownPasses[int(b)] = in.ReadByte()
			}
		}
		...
		if build.AddedIn(9056) {
			...
			if g.TargetFlags&DestinationLocation != 0 {
				g.DestinationLocationCount = in.ReadByte()
			}
