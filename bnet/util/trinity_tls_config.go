...
// return the TLS server config necessary to
// make a BattleNet server compatible with Arctium-launched clients
func GetTrinityTLSConfig() *tls.Config {
	rsa_key_pair, err := tls.X509KeyPair(
		[]byte(Cert),
		[]byte(Key),
	)
	if err != nil {
	...
	config := &tls.Config{
		Certificates: []tls.Certificate{rsa_key_pair},
	}
	return config
}
