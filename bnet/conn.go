...
func (c *Conn) Dispatch(header *p.Header, data []byte) {
	...
	if header.GetServiceId() == ResponseService {
		...
		if ch := c.pendingRequests[header.GetToken()]; ch != nil {
			...
			log.Warn("Unknown token", header.GetToken())
		}
		c.pendingRequestsL.Unlock()
		return
	}
